...
%         end
    ...
    %         ubg = [ubg;0];
        ...
        if(i==2) 
            x_i = [vh(i-1), v(i-1,M)];
            [gp_pred{1}, ~] = gp_model(x_i);
            mean = gp_pred{1};
    
            g = [g; -vh_gp(i) + vh(i) + mean]; 
            lbg = [lbg;0];
            ubg = [ubg;0];
        end
        if(i==3) 
    ...
    %         ubg = [ubg;0];
        ...
        if(i>2)
            ...
            g = [g; -ph(i) + ph(i-1) + dt*vh(i-1) + dt*mean];
            lbg = [lbg;0];
            ubg = [ubg;0];
    
            g = [g; -ph_sigma(i) + ph_sigma(i-1) + dt*dt*sigma];
            lbg = [lbg;0];
            ubg = [ubg;0];
    ...
    % Get to solver
    options = struct('ipopt', struct('tol', 1e-4, 'acceptable_tol', 1e-2, 'max_iter', 100, 'linear_solver', 'mumps','hessian_approximation','limited-memory',...
